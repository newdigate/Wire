...
void setup() {
	...
	// 1. Bus scan: the EEPROM (0x50) must be the ONLY device that ACKs. A broken
	// NACK-detection (racing TDF against NDF) makes every address falsely ACK, so
	// scan the whole range and assert exactly one responder.
	int found = -1, scan_count = 0;
	for (uint8_t a = 1; a < 0x7F; a++) {
		Wire.beginTransmission(a);
		if (Wire.endTransmission() == 0) { if (found < 0) found = a; scan_count++; }
	}
	Serial1.print("scan_found=0x"); if (found >= 0) print_hex((uint8_t)found); else Serial1.print("NONE");
	Serial1.println();
	Serial1.print("scan_count="); Serial1.println(scan_count);

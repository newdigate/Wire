...
// Single-byte response: QEMU's master reads a whole requestFrom in one step without
// yielding to the slave ISR, so multi-byte reads can't be gated here (they're
// hardware-verified against an MKR Zero master). This still proves onRequest fires.
void onReq(void)   { Wire1.write((uint8_t)0x11); }
...
void setup() {
	...
	// Master reads 1 byte from the slave -> onRequest fires and returns 0x11.
